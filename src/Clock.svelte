<script lang="ts">
  ...
  $: formatedMinute = String(Math.floor((msecond % 3600000) / 60000)).padStart(
    2,
    "0"
  );
  $: formatedSecond = String(Math.floor((msecond % 60000) / 1000)).padStart(
    2,
    "0"
  );
  ...
  function syncTypedTime() {
    let endResult: number =
      parseInt(formatedHour) * 3600000 +
      parseInt(formatedMinute) * 60000 +
      parseInt(formatedSecond) * 1000;
  ...
  function scrollTime(event: WheelEvent, multiplier: number) {
    if (currentState !== ClockState.countdown) return;
...
<main {style}>
  ...
  {/if}
  <input
    bind:value={formatedHour}
    style="grid-area: hour-text"
    type="number"
    class="time-text"
    class:input-text={isTyping}
    on:wheel={(e) => scrollTime(e, 3600000)}
    on:keypress={onType}
    on:blur={() => {
      syncTypedTime();
      isTyping = false;
    ...
    readonly={currentState !== ClockState.countdown}
  />
  <input
    bind:value={formatedMinute}
    style="grid-area: min-text"
    type="number"
    class="time-text"
    class:input-text={isTyping}
    on:wheel={(e) => scrollTime(e, 60000)}
    on:keypress={onType}
    on:blur={() => {
      syncTypedTime();
      isTyping = false;
  ...
  />
  <input
    bind:value={formatedSecond}
    style="grid-area: sec-text"
    type="number"
    class="time-text"
    class:input-text={isTyping}
    on:wheel={(e) => scrollTime(e, 1000)}
    on:keypress={onType}
    on:blur={() => {
      syncTypedTime();
      isTyping = false;
